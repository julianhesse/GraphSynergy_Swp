...
def main(config):
    ...
    # get the database name from the config file to store the results accurately
    used_db = config['name']
    test_results = []
    ...
    test_results.to_csv(config.log_dir / f'baseline_{used_db}_test_results.csv', index=False)
    print(test_results)
