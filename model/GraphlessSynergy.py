...
class GraphlessSynergy(BaseModel):
    ...
    def __init__(self, 
        ...
        if not self.use_graph:
            ...
            self.drug1_neighbor_weights = nn.ParameterList([
                    nn.Parameter(torch.randn((n_memory, self.emb_dim))) for _ in range(self.n_hop)
                ])
    ...
    def forward(self,
        ...
        if self.use_graph:
            ...
            cell_i_list = self._interaction_aggregation(cell_embeddings, self.cell_neighbor_weights)
            drug1_i_list = self._interaction_aggregation(drug1_embeddings, self.drug1_neighbor_weights)
            drug2_i_list = self._interaction_aggregation(drug2_embeddings, self.drug2_neighbor_weights)
